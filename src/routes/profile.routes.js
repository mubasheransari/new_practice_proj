...
router.get('/', authRequired, (req, res) => {
  const u = db.prepare(`
    SELECT
      userCode AS id,   -- 👈 8-char alphanumeric id
      firstName,
      lastName,
      email,
      role,
...
//   const updated = db.prepare('SELECT id, firstName, lastName, email, role, createdAt FROM users WHERE id=?')
//     .get(req.user.id);
//   res.json(updated);
// });
...
// routes/profile.routes.js  (or profile.js)
// const express = require('express');
// const { db } = require('../db');
// const { authRequired } = require('../middleware/auth');
...
// router.get('/', authRequired, (req, res) => {
//   const u = db
//     .prepare('SELECT id, firstName, lastName, email, role, createdAt FROM users WHERE id=?')
//     .get(req.user.id);
//   res.json(u);
// });
